...
// ✅ Download file from Google Drive (temporary save)
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        // download from Google Drive (temp save)
        ...
        await Image.create({
          ...
          localPath: result.secure_url, // 🟢 Cloudinary URL
          ...placeDetails,
        });
...
const getImageStatsByMonth = async (req, res) => {
  try {
    ...
    const uniqueUploaders = [
      ...new Set(monthlyStats.map((s) => s.uploadedBy).filter(Boolean)),
    ];
    res.status(200).json({ stats: monthlyStats, uniqueUploaders });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Failed to get monthly stats", error: err.message });
  }
};
...
const getImageStatsByYear = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to get yearly stats", error: err.message });
  }
};
...
const getImageStatsByDay = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to get daily stats", error: err.message });
  }
};
